...
func SetupRouter(cfg *config.Config) *gin.Engine {
	...
	// Repositories (via factory — currently backed by MongoDB)
	db := database.DB
	userRepo := repositories.NewUserRepository(db)
	challengeRepo := repositories.NewChallengeRepository(db)
	submissionRepo := repositories.NewSubmissionRepository(db)
	teamRepo := repositories.NewTeamRepository(db)
	teamInvitationRepo := repositories.NewTeamInvitationRepository(db)
	notificationRepo := repositories.NewNotificationRepository(db)
	hintRepo := repositories.NewHintRepository(db)
	contestRepo := repositories.NewContestRepository(db)
	contestEntityRepo := repositories.NewContestEntityRepository(db)
	contestRoundRepo := repositories.NewContestRoundRepository(db)
	roundChallengeRepo := repositories.NewRoundChallengeRepository(db)
	writeupRepo := repositories.NewWriteupRepository(db)
	auditLogRepo := repositories.NewAuditLogRepository(db)
	achievementRepo := repositories.NewAchievementRepository(db)
	scoreAdjustmentRepo := repositories.NewScoreAdjustmentRepository(db)
	teamContestRegistrationRepo := repositories.NewTeamContestRegistrationRepository(db)
